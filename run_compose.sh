...
EOF_init
    ...
    if [ -n "$cho" ]; then
        if [ "$cho" = 'y' ] || [ "$cho" = 'Y' ]; then
            read -r -p "输入集群编排文件名： " docker_compose_file_new
...
function docker_image_save() {
    ...
    "")
        ...
        grep "image:" $docker_compose_file | sed 's/#//' | awk '{print $2}' | sort -u | while IFS= read -r line; do
            j=$(echo "$line" | sed "s/:/_/g;s/\//-/g")
            if docker "$docker_remote_arg" image save "$line" >./images_bak_"$now_time"/"$j"; then
                echo "$line 已导出为 $j"
            else
                echo "$line 导出失败"
            fi
    ...
    "-b")
        ...
        grep "FROM" "$dockerfile_file" | awk '{print $NF}' | sort -u | while IFS= read -r line; do
            if [ "${line:0:1}" == '$' ]; then
                echo "Dockerfile 内 FROM 的镜像是个变量 $line，尝试使用 $0 save -c 备份 $docker_compose_file 中的 base_image"
            else
                j=$(echo "$line" | sed "s/:/_/g;s/\//-/g")
                if docker "$docker_remote_arg" image save "$line" >./base_images_bak_"$now_time"/"$j"; then
                    echo "$line 已导出为 $j"
                else
                    echo "$line 导出失败"
                fi
    ...
    "-c")
        ...
        grep "base_image=" $docker_compose_file | sed 's/#//' | awk '{print $2}' | sort -u | cut -d '=' -f 2 | while IFS= read -r line; do
            j=$(echo "$line" | sed "s/:/_/g;s/\//-/g")
            if docker "$docker_remote_arg" image save "$line" >./base_images_from_compose_bak_"$now_time"/"$j"; then
                echo "$line 已导出为 $j"
            else
                echo "$line 导出失败"
            fi
...
function docker_service_choose() {
    ...
    i=0
    echo -e "序号  服务\n----------"
    for docker_service_name in $(docker-compose "$docker_stack_name_arg" "$docker_remote_arg" "$docker_compose_file_arg" ps --services | sort); do
        i=$((i + 1))
        if [ $i -lt 10 ]; then
            j=' '$i
...
function docker_compose_port() {
    ...
    $1)
        ...
        if sed -i "s/$original_value/$1/" $docker_compose_file &>/dev/null; then
...
function docker_compose_bash() {
    docker_service_choose
    if ! docker-compose "$docker_stack_name_arg" "$docker_remote_arg" "$docker_compose_file_arg" exec "$docker_service_choice" bash; then
        docker-compose "$docker_stack_name_arg" "$docker_remote_arg" "$docker_compose_file_arg" exec "$docker_service_choice" sh
    fi
}
